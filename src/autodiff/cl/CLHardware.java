...
public final class CLHardware implements Serializable {
	...
	public final CLDevice getDevice(final long preferredType) {
		for (final CLPlatform platform : this.getPlatforms()) {
			final CLDevice device = platform.getDevice(preferredType);
			
			if (preferredType == device.getType()) {
				return device;
			}
		}
		
		return this.getPlatforms()[0].getDevices()[0];
	}
